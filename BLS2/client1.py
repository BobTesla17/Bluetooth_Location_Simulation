...
Raspi1 = Mybluetooth(0.1,X1,Y1,A1,n1)
...
while True:
    target1_rssi = Raspi1.get_RSSI(Tarx[0],Tary[0],Spex[0],Spey[0])
    target1_rssistr = str(target1_rssi)
    message1 = ID+',[\''+MAC1+'\', '+ target1_rssistr+']'
    print(message1)
    target２_rssi = Raspi1.get_RSSI(Tarx[1],Tary[1],Spex[1],Spey[1])
    target２_rssistr = str(target2_rssi)
    message2 = ID+',[\''+MAC2+'\', '+ target2_rssistr+']'
    print(message2)
    target3_rssi = Raspi1.get_RSSI(Tarx[2],Tary[2],Spex[2],Spey[2])
